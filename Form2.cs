...
namespace SGenerator_Pico
{
    public partial class Form2 : Form
    {
        ...
        private string StreamFile = "sonuc.txt";
         ...
         ****************************************************************************/
        void SetDefaults(double handle)
        {
            for (int i = 0; i < _channelCount; i++) // reset channels to most recent settings
            {
                uint status = Imports.SetChannel(Convert.ToInt16(handle),
         ...
         ****************************************************************************/
        void SetDigitals(double handle)
        {
            ...
            {
                status = Imports.SetDigitalPort(Convert.ToInt16(handle), port, enabled, logicLevel);
            }
         ...
         ****************************************************************************/
        void DisableDigital(double handle)
        {
            Imports.Channel port;
            uint status;
            ...
            {
                status = Imports.SetDigitalPort(Convert.ToInt16(handle), port, 0, 0);
            }
        }
        ...
        ****************************************************************************/
        void DisableAnalogue(double handle)
        {
            uint status;
            ...
            {
                status = Imports.SetChannel(Convert.ToInt16(handle), Imports.Channel.ChannelA + i, 0, 0, 0, 0);
        ...
        {
            int a = (raw * inputRanges[ch]) / _maxValue;
            return a;
        ...
        ****************************************************************************/
        void ClearDataBuffers(double handle)
        {
            int i;
            uint status;
            ...
            {
                status = Imports.SetDataBuffers(Convert.ToInt16(handle), (Imports.Channel)i, null, null, 0, 0, Imports.RatioMode.None);
            ...
            {
                status = Imports.SetDataBuffers(Convert.ToInt16(handle), i + Imports.Channel.PS2000A_DIGITAL_PORT0, null, null, 0, 0, Imports.RatioMode.None);
         ...
         ****************************************************************************/
        void BlockDataHandler(string text, int offset, Imports.Mode mode, double handle)
        {
            ...
            if (mode == Imports.Mode.ANALOGUE || mode == Imports.Mode.MIXED)
            {
                for (int i = 0; i < _channelCount; i++)
                {
                    ...
                    status = Imports.SetDataBuffers(Convert.ToInt16(handle), (Imports.Channel)i, maxBuffers, minBuffers, (int)sampleCount, 0, Imports.RatioMode.None);
            ...
            if (mode == Imports.Mode.DIGITAL || mode == Imports.Mode.MIXED)
            {
                for (int i = 0; i < _digitalPorts; i++)
                {
                    ...
                    status = Imports.SetDataBuffer(Convert.ToInt16(handle), i + Imports.Channel.PS2000A_DIGITAL_PORT0, digiBuffer, (int)sampleCount, 0, Imports.RatioMode.None);
            ...
            */
            ...
            while (Imports.GetTimebase(Convert.ToInt16(handle), _timebase, (int)sampleCount, out timeInterval, _oversample, out maxSamples, 0) != 0)
            {
            ...
            Imports.RunBlock(Convert.ToInt16(handle), 0, (int)sampleCount, _timebase, _oversample, out timeIndisposed, 0, _callbackDelegate, IntPtr.Zero);
            ...
            {
                short overflow;
                Imports.GetValues(Convert.ToInt16(handle), 0, ref sampleCount, 1, Imports.DownSamplingMode.None, 0, out overflow);
            ...
            Imports.Stop(Convert.ToInt16(handle));
         ...
         ****************************************************************************/
        private void RapidBlockDataHandler(ushort nRapidCaptures, double handle)
        {
            uint status;
            ...
            _callbackDelegate = BlockCallback;
            status = Imports.RunBlock(Convert.ToInt16(handle),
                        0,
                        (int)numSamples,
                        _timebase,
                        _oversample,
            ...
            for (ushort segment = 0; segment < nRapidCaptures; segment++)
            {
                ...
                for (short channel = 0; channel < numChannels; channel++)
                {
                    if (_channelSettings[channel].enabled)
                    {
                        ...
                        status = Imports.SetDataBuffer(Convert.ToInt16(handle),
            ...
            status = Imports.GetValuesRapid(Convert.ToInt16(handle), ref numSamples, 0, (ushort)(nRapidCaptures - 1), 1, Imports.DownSamplingMode.None, overflows);
        ...
        ****************************************************************************/
        uint SetTrigger(Imports.TriggerChannelProperties[] channelProperties,
                        ...
                        short nDigitalDirections, double handle)
        {
            uint status;

            if ((status = Imports.SetTriggerChannelProperties(Convert.ToInt16(handle), channelProperties, nChannelProperties, auxOutputEnabled, autoTriggerMs)) != 0)
            {
                return status;
            }

            if ((status = Imports.SetTriggerChannelConditions(Convert.ToInt16(handle), triggerConditions, nTriggerConditions)) != 0)
            {
                return status;
            }
            ...
            if ((status = Imports.SetTriggerChannelDirections(Convert.ToInt16(handle),
            ...
            if ((status = Imports.SetTriggerDelay(Convert.ToInt16(handle), delay)) != 0)
            {
                return status;
            }
            ...
            status = Imports.SetPulseWidthQualifier(Convert.ToInt16(handle), pwq.conditions,
                                                    pwq.nConditions, pwq.direction,
                                                    pwq.lower, pwq.upper, pwq.type);

            if (_digitalPorts > 0)
            {
                if ((status = Imports.SetTriggerDigitalPort(Convert.ToInt16(handle), digitalDirections, nDigitalDirections)) != 0)
                {
                    return status;
                }
            }
        ...
        ****************************************************************************/
        void CollectBlockImmediate(double handle)
        {
            ...
            /* Trigger disabled	*/
            SetTrigger(null, 0, null, 0, null, null, 0, 0, 0, null, 0, Convert.ToInt16(handle));

            BlockDataHandler("First 10 readings", 0, Imports.Mode.ANALOGUE, Convert.ToInt16(handle));
        }
        ...
        ****************************************************************************/
        void CollectBlockRapid(double handle)
        {
            ...
            status = Imports.SetNoOfRapidCaptures(Convert.ToInt16(handle), numRapidCaptures);
            ...
            int maxSamples;
            status = Imports.MemorySegments(Convert.ToInt16(handle), numRapidCaptures, out maxSamples);
            ...
            SetTrigger(null, 0, null, 0, null, null, 0, 0, 0, null, 0, handle);

            RapidBlockDataHandler(numRapidCaptures, handle);
        }
       ...
       ****************************************************************************/
        void CollectBlockTriggered(double handle)
        {
             ...
             * Rising edge
             * Threshold = 1000mV */
            SetTrigger(sourceDetails, 1, conditions, 1, directions, null, 0, 0, 0, null, 0, handle);

            BlockDataHandler("Ten readings after trigger", 0, Imports.Mode.ANALOGUE, Convert.ToInt16(handle));
        }

        /****************************************************************************
        * Initialise unit' structure with Variant specific defaults
        ****************************************************************************/
        void GetDeviceInfo(double handle)
        {
            uint status = 0;
            ...
            if (handle >= 0)
            {
                ...
                {
                    short requiredSize;
                    Imports.GetUnitInfo(Convert.ToInt16(handle), line, 80, out requiredSize, i);
                ...
                // Find max ADC count
                status = Imports.MaximumValue(Convert.ToInt16(handle), out _maxValue);
         ...
         ****************************************************************************/
        void SetVoltages(double handle)
        {
            bool valid = false;
            ...
            SetDefaults(handle);  // Set defaults now, so that if all but 1 channels get switched off, timebase updates to timebase 0 will work
        }
         ...
         *
         ****************************************************************************/
        void SetTimebase(double handle)
        {
            int timeInterval;
            int maxSamples;
            bool valid = false;
            ...
            while (Imports.GetTimebase(Convert.ToInt16(handle), _timebase, BUFFER_SIZE, out timeInterval, 1, out maxSamples, 0) != 0)
            {
        ...
        ***************************************************************************/
        void StreamDataHandler(uint preTrigger, Imports.Mode mode, double handle)
        {
            ...
            switch (mode)
            {
                case Imports.Mode.ANALOGUE:
                    ...
                    for (int channel = 0; channel < _channelCount * 2; channel += 2) // create data buffers
                    {
                        ...
                        status = Imports.SetDataBuffers(Convert.ToInt16(handle), (Imports.Channel)(channel / 2), buffers[channel], buffers[channel + 1], (int)tempBufferSize, 0, Imports.RatioMode.Aggregate);
                    }
                ...
                case Imports.Mode.AGGREGATED:
                    ...
                    for (int port = 0; port < _digitalPorts * 2; port += 2) // create data buffers
                    {
                        ...
                        status = Imports.SetDataBuffers(Convert.ToInt16(handle), (Imports.Channel)(port / 2) + 0x80, digiBuffers[port], digiBuffers[port + 1], (int)tempBufferSize, 0, Imports.RatioMode.Aggregate);
                    }
                ...
                case Imports.Mode.DIGITAL:
                    ...
                    for (int port = 0; port < _digitalPorts; port++) // create data buffers
                    {
                        ...
                        status = Imports.SetDataBuffer(Convert.ToInt16(handle), (Imports.Channel)port + 0x80, digiBuffers[port], (int)tempBufferSize, 0, Imports.RatioMode.None);
                    }
            ...
            status = Imports.RunStreaming(Convert.ToInt16(handle), ref sampleInterval, timeUnits, preTrigger, postTrigger, autoStop, downsampleRatio, ratioMode, tempBufferSize);
            ...
            while (!_autoStop)
            {
                ...
                status = Imports.GetStreamingLatestValues(Convert.ToInt16(handle), StreamingCallback, (System.IntPtr)mode);
                    ...
                    for (uint i = _startIndex; i < (_startIndex + _sampleCount); i++)
                    {
                        if (mode == Imports.Mode.ANALOGUE)
                        {
                            ...
                            for (int ch = 0; ch < _channelCount * 2; ch += 2)
                            {
                                ...
                                {
                                    sb.AppendFormat("{0,12} {1,14} {2,12} {3,12} {4,12}",
                                                    (char)('A' + (ch / 2)),
        ...
        *  from the unit (start collecting immediately)
        ***************************************************************************/
        void CollectStreamingImmediate(double handle)
        {
            SetDefaults(handle);
            ...
            StreamDataHandler(0, Imports.Mode.ANALOGUE, handle);
        }
        ...
        ***************************************************************************/
        void CollectStreamingTriggered(double handle)
        {
            short triggerVoltage = mv_to_adc(1000, (short)_channelSettings[(int)Imports.Channel.ChannelA].range); // ChannelInfo stores ADC counts
            ...
            StreamDataHandler(100000, Imports.Mode.ANALOGUE, handle);
        }
        ...
        ***************************************************************************/
        void DigitalBlockImmediate(double handle)
        {
        ...
        * Collect a block of data from the digital ports with triggering disabled
        ***************************************************************************/
        void DigitalBlockTriggered(double handle)
        {
            ...
            SetTrigger(null, 0, conditions, 1, null, null, 0, 0, 0, digitalDirections, 2, handle);
        ...
        ****************************************************************************/
        void ANDAnalogueDigitalTriggered(double handle)
        {
            ...
            * Ch15 High */
            ...
            SetTrigger(sourceDetails, 1, conditions, 1, directions, null, 0, 0, 0, digitalDirections, 2, handle);

            BlockDataHandler("Ten readings after trigger", 0, Imports.Mode.MIXED, handle);

            DisableAnalogue(handle);
        }
        ...
        *  to collect a block of data.
        ****************************************************************************/
        void ORAnalogueDigitalTriggered(double handle)
        {
            ...
            SetTrigger(sourceDetails, 1, conditions, 2, directions, null, 0, 0, 0, digitalDirections, 2, handle);
         ...
         ***************************************************************************/
        void DigitalStreamingImmediate(double handle)
        {
            ...
            StreamDataHandler(0, Imports.Mode.DIGITAL, handle);
        }
        ...
        * Streams data from the digital ports with triggering disabled
        ***************************************************************************/
        void DigitalStreamingAggregated(double handle)
        {
            ...
            StreamDataHandler(0, Imports.Mode.AGGREGATED, handle);
        }
       ...
       *  
       **************************************************************************************/
        void SetSignalGenerator(double handle)
        {
            ...
            status = Imports.SigGenArbitraryMinMaxValues(Convert.ToInt16(handle), out minArbitraryWaveformValue, out maxArbitraryWaveformValue, out minArbitraryWaveformSize, out maxArbitraryWaveformSize);
            ...
            if (waveformSize > 0)
            {
                ...
                Imports.SigGenFrequencyToPhase(Convert.ToInt16(handle), frequency, Imports.IndexMode.PS2000A_SINGLE, waveformSize, out phase);

                status = Imports.SetSigGenArbitrary(Convert.ToInt16(handle),
                                                    0,
                                                    pkToPk,
                                                    (uint)phase,
                                                    (uint)phase,
            ...
            {
                status = Imports.SetSigGenBuiltIn(Convert.ToInt16(handle), offset, pkToPk, (short)waveform, (float)frequency, (float)frequency, 0, 0, 0, operation, 0, 0, 0, 0, 0);
         ...
         ****************************************************************************/
        public void DigitalMenu(double handle)
        {
            char ch;

            DisableAnalogue(handle);                  // Disable the analogue ports
            SetDigitals(handle);                      // Enable digital ports & set logic level
            ...
            while (ch != 'X')
            {
                ...
                {
                    case 'B':
                        DigitalBlockImmediate(handle);
                        break;

                    case 'T':
                        DigitalBlockTriggered(handle);
                        break;

                    case 'A':
                        ANDAnalogueDigitalTriggered(handle);
                        break;

                    case 'O':
                        ORAnalogueDigitalTriggered(handle);
                        break;

                    case 'S':
                        DigitalStreamingImmediate(handle);
                        break;

                    case 'V':
                        DigitalStreamingAggregated(handle);
                        break;
                }
            }
            DisableDigital(handle);       // disable the digital ports when we're finished
        }

        /****************************************************************************
        * Run - show menu and call user selected options
        ****************************************************************************/
        public void Run(double handle)
        {
            // setup devices
            ...
            char ch = ' ';
            CollectStreamingImmediate(handle);
            //while (ch != 'X')
            //{
            //    DisplaySettings();
        ...
        //public PS2000ACSConsole(short handle)
        //{
        //    _handle = handle;
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            string target = @"sonuc.txt";
            StreamReader streamReader = File.OpenText(target);
